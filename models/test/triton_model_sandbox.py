...
def create_argparser():
    parser = argparse.ArgumentParser(
        description="Run Triton Inference Server container"
    )
...
def build_container_config(args):
    ...
    return {
        ...
        "volumes": {host_gcloud_path: {"bind": container_gcloud_path, "mode": "ro"}},
        "environment": {
...
def run_container(docker_client, config, detach):
    try:
        ...
        if detach:
            print(
                f"🚀 Triton Server running in background (container ID: {container.short_id})"
            )
...
def main():
    ...
    print(
        f"🔧 Starting Triton Server ({'detached' if args.detach else 'foreground'} mode)"
    )
